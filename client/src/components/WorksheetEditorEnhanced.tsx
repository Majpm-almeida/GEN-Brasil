...
import { countWords, getWorksheetTextDistribution } from "@/lib/worksheetTextMetrics";
...
const emptyDraft = (): WorksheetDraft => ({
  classification: "", selectedEventIds: [], decisiveEventIds: [], testEntries: {}, testResults: {}, testSufficiency: {}, centralJudgment: "", evidenceBasis: "", limitsAndAlternatives: "", clarificationNeeded: "", integrationInput: "",
});
...
function ParagraphField({ number, title, value, onChange, hint }: { number: string; title: string; value: string; onChange: (value: string) => void; hint: string }) {
  ...
  const status = words === 0 ? "Ainda não iniciado" : words < 70 ? "Abaixo da faixa sugerida" : words > 130 ? "Acima da faixa sugerida" : "Faixa equilibrada";
  const tone = words === 0 ? "text-muted-foreground" : words < 70 ? "text-amber-800" : words > 130 ? "text-rose-700" : "text-emerald-700";
  const barTone = words === 0 ? "bg-slate-300" : words < 70 ? "bg-amber-400" : words > 130 ? "bg-rose-500" : "bg-emerald-500";
  const barWidth = Math.min(100, Math.round((words / 130) * 100));
  return <div className="rounded-xl border border-border/75 bg-background/55 p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex gap-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-secondary text-xs font-bold text-primary">{number}</span><div><Label className="text-base font-semibold text-primary">{title}</Label><p className="mt-1 text-sm leading-6 text-muted-foreground">{hint}</p></div></div><span className="text-xs text-muted-foreground">{words} palavras</span></div><Textarea value={value} onChange={(event) => onChange(event.target.value)} className="mt-4 min-h-32 resize-y bg-card" placeholder="Redija o parágrafo analítico do GT." /><div className="mt-3 grid gap-2 text-xs sm:grid-cols-[1fr_auto]"><div className="flex items-center gap-2"><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-200"><div className={`h-full rounded-full ${barTone}`} style={{ width: `${barWidth}%` }} /></div><span className="whitespace-nowrap text-muted-foreground">Faixa: 70–130</span></div><span className={`font-semibold ${tone}`}>{status}</span></div></div>;
}

function TextDistributionSummary({ draft }: { draft: WorksheetDraft }) {
  const metrics = getWorksheetTextDistribution(draft);
  const denominator = Math.max(1, metrics.largest);
  const status = !metrics.allStarted ? "Preencha os cinco parágrafos para comparar a distribuição." : metrics.balanced ? "Distribuição equilibrada entre os cinco parágrafos." : "Distribuição a revisar: há diferença relevante entre os parágrafos.";
  const tone = !metrics.allStarted ? "border-slate-200 bg-slate-50 text-slate-700" : metrics.balanced ? "border-emerald-200 bg-emerald-50 text-emerald-900" : "border-amber-300 bg-amber-50 text-amber-950";
  return <section className={`rounded-xl border p-4 ${tone}`}><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-sm font-semibold">Distribuição dos cinco parágrafos</p><p className="mt-1 text-xs leading-5 opacity-80">Compare o volume de redação de cada bloco antes de finalizar a ficha.</p></div><div className="flex gap-3 text-xs"><span><strong>{metrics.total}</strong> palavras</span><span><strong>{Math.round(metrics.average)}</strong> por parágrafo</span></div></div><div className="mt-4 grid gap-2 sm:grid-cols-5">{metrics.entries.map((entry, index) => <div key={entry.label} className="rounded-lg border border-current/15 bg-white/55 p-2.5"><div className="flex justify-between gap-2 text-[0.68rem] font-semibold"><span>P{index + 1}</span><span>{entry.words}</span></div><p className="mt-1 truncate text-[0.65rem] opacity-75" title={entry.label}>{entry.label}</p><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-current/15"><div className="h-full rounded-full bg-current" style={{ width: `${Math.round((entry.words / denominator) * 100)}%` }} /></div></div>)}</div><p className="mt-3 text-xs font-medium">{status}</p></section>;
}
