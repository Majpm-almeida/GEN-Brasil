...
export default function AccessGate({ children }: { children: React.ReactNode }) {
  ...
  return <BrandedShell><Card className="relative z-10 w-full max-w-2xl border-primary/10 bg-card/95 shadow-2xl shadow-primary/10"><CardContent className="p-8 sm:p-10"><div className="grid h-12 w-12 place-items-center rounded-2xl bg-amber-100 text-amber-800">{rejected ? <ShieldCheck className="h-6 w-6" /> : <Clock3 className="h-6 w-6" />}</div><p className="eyebrow mt-7">SOLICITAÇÃO DE ACESSO</p><h1 className="mt-3 font-serif text-3xl text-primary">{rejected ? "Solicitação em análise pela coordenação" : `Bem-vindo(a), ${user.name?.split(" ")[0] ?? "participante"}.`}</h1><p className="mt-4 max-w-xl text-base leading-7 text-muted-foreground">{rejected ? "A solicitação não foi liberada nesta etapa. Caso considere necessário, entre em contato com a coordenação do curso." : "Sua conta Google foi identificada e sua solicitação foi registrada. Enquanto a aprovação do gestor é concluída, você pode conhecer o propósito desta jornada acadêmica."}</p>{!rejected && <div className="mt-7 grid gap-4 rounded-2xl border border-primary/10 bg-secondary/45 p-5 sm:grid-cols-3"><div><p className="text-sm font-semibold text-primary">Analisar</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Aplicar as lentes de Guerra Híbrida, Lawfare e Segurança Transnacional.</p></div><div><p className="text-sm font-semibold text-primary">Integrar</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Conectar evidências, incertezas e recomendações estratégicas.</p></div><div><p className="text-sm font-semibold text-primary">Entregar</p><p className="mt-1 text-xs leading-5 text-muted-foreground">Organizar a síntese e os quatro slides finais do GT.</p></div></div>}<div className="mt-8 flex flex-col gap-3 sm:flex-row"><Button className="h-11 flex-1" disabled={access.isFetching} onClick={() => void access.refetch()}><CheckCircle2 className="mr-2 h-4 w-4" />{access.isFetching ? "Verificando..." : "Verificar aprovação"}</Button><Button variant="outline" className="h-11" onClick={() => void logout()}><LogOut className="mr-2 h-4 w-4" />Sair</Button></div><p className="mt-4 text-center text-xs text-muted-foreground">{rejected ? "O acesso permanece indisponível até nova liberação." : "Você receberá um e-mail de boas-vindas quando o acesso for aprovado."}</p></CardContent></Card></BrandedShell>;
}
